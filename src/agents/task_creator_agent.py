...
class TaskCreatorAgent:
    ...
    async def execute_hallucination_task(
        self, text: str, context: str = ""
    ) -> Optional[EvaluationResult]:
        """
        Execute hallucination detection task via hallucination agent.
        Pay hallucination agent upon completion.
        """
        detection = await self.hallucination_agent.detect_hallucinations(text, context)

        if detection:
            # Pay hallucination agent for work completed
            self.router.authorize_payment(
                from_agent=self.agent_id,
                to_agent=self.hallucination_agent.agent_id,
                amount=HallucinationAgent.DETECTION_PRICE,
                reason="hallucination_detection_task",
            )
    ...
    def get_orchestrator_stats(self) -> dict:
        ...
        return {
            ...
            "hallucination_agent": self.hallucination_agent.get_stats(),
            "payment_flow": self.router.get_payment_flow_summary(),
            "agent_accounts": {
                agent_id: self.router.get_agent_stats(agent_id)
                for agent_id in [
                    self.quality_agent.agent_id,
                    self.settlement_agent.agent_id,
                    self.hallucination_agent.agent_id,
                    self.agent_id,
                ]
            },
        }
    ...
    def reset(self):
        ...
        self.hallucination_agent.reset_stats()
        self.router.reset()
